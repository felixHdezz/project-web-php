					...
					<div id="CatalogoProductos">
						<div id="CatalogoProduc">
							<?php
							 $conexion= new Conexion();
							 if (isset($_GET['clave'])) {
							 	$consulta=$conexion->Consulta("SELECT * FROM tbl_productos where Id_categoria=".$_GET['clave']);
							 	while ($fila=mysql_fetch_array($consulta)) {
							 	$ids=$fila[0];
							 	$nombre=$fila[1];
							 	$imagen=$fila[2];
							 ...
							 <div class="caja">
							 	<form  id="form1" name="form1" method="POST" action="AgergarCarrito.php">
							 		...
							 		<div id="opc"><a href="AgergarCarrito.php?id=<?php echo $ids ?>">Carrito</a>
							 		<a href="Productodetalle.php?id=<?php echo $ids ?>">Detalle</a></div>
							 	</form>
							 </div>
							 <?php
							 }
							 }else{
							 $consulta=$conexion->Consulta("SELECT * FROM tbl_productos ORDER BY Id_Producto ASC ");
							 while ($fila=mysql_fetch_array($consulta)) {
							 	$ids=$fila[0];
							 	$nombre=$fila[1];
							 	$imagen=$fila[2];
